import { useReducer, useEffect, useState, createRef } from "react";
...
function parseQueryFilters(location) {
    ...
    const filterValues = {
        ...
        price: '0, 100000000', // filter price from
...
function reducer(state, action) {
    switch (action.type) {
        ...
        case 'RESET_FILTER':
            return {
                ...state,
                filters: { page: 1, limit: 26, recomendedOnly: false, price: '0,100000000' },
            };
        default:
            throw new Error();
    }
...
function HotelProductPage() {
    ...
    const handleSuccess = (position) => {
        ...
        setTimeout(() => {
            const latitude = position.coords.latitude;
            const longitude = position.coords.longitude;
            alert("Kami Menemukan Hotel Terdekat dari Anda");
            homeApi.searchHotelTerdekat(access_token, state.options.keyword, latitude, longitude).then(res => {
                if (res.success) {
                    setCoordinates({ lng: Number(res.data.productOptions[0]?.detail.longitude), lat: Number(res.data.productOptions[0]?.detail.latitude) })
                    dispatch({ type: 'FETCH_SEARCHHOTEL_SUCCESS', payload: res.data.productOptions })
                } else {
                    ...
                    {showMaps ?
                        <>
                            <div className="col-12">
                                <h5 className="text-primary" style={{ cursor: 'pointer' }} onClick={handleShowMap}>
                                    <Icon icon={ArrowLeft} /> Kembali</h5>
                            </div>
                            <div id={'hotels'} className="col-5" style={{
                                maxHeight: '70vh',
                                overflowY: 'auto'
                            }}>
                                {
                            ...
                            {!isTabletOrMobile &&
                                <div className="col-3">
                                    <div onClick={handleShowMap} style={{
                                        ...
                                        <div className="mb-2" style={{ cursor: 'pointer', position: 'relative', background: "url('https://cdn.masterdiskon.com/masterdiskon/assets/front/img/icon/map-view-bg.svg') no-repeat center center/cover", borderRadius: '10px', height: '100px' }}>
                                            <div style={{
                                                position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)'
                                            }} className="text-center text-primary bg-light w-50 font-weight-bold p-2 rounded"> <Icon icon={MapIcon} /> MAP VIEW</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="row mb-3 align-items-center justify-content-center">
                                        <div className="col-6">
                                            <h6 className=" font-weight-bold">
                                                <span class="text-primary " style={{ fontSize: "20px" }}> {searchResult?.name || state.options.keyword} {ReactTitle(searchResult?.name || state.options.keyword)}
                                                </span> ada <span class="text-primary ">{state.meta.total || 0}</span> akomodasi ditemukan
                                            </h6>
                                        </div>
                                        <div className="col-6">
...
const dataFilters = [
    { name: 'Search Hotel Name ', value: '', type: 'search', slug: 'search' },
    // {
    //     name: '', slug: 'recomendedOnly', value: [], type: 'check', items: [
    //         { slug: 'true', name: 'Rekomendasi Hotel' },
    //     ]
    // },
    { name: 'Range Harga', slug: 'price', value: [], type: 'range', defaultValue: [0, 100000000] },
    {
        name: 'Rating Hotel', value: [], type: 'check', slug: 'class',
        items: [
            { slug: '5', name: 'Bintang 5', icon: 'https://cdn.masterdiskon.com/masterdiskon/icon/fe/star.png', isImg: true, loop: true, className: 'text-warning' },
            { slug: '4', name: 'Bintang 4', icon: 'https://cdn.masterdiskon.com/masterdiskon/icon/fe/star.png', isImg: true, loop: true, className: 'text-warning' },
            { slug: '3', name: 'Bintang 3', icon: 'https://cdn.masterdiskon.com/masterdiskon/icon/fe/star.png', isImg: true, loop: true, className: 'text-warning' },
            { slug: '2', name: 'Bintang 2', icon: 'https://cdn.masterdiskon.com/masterdiskon/icon/fe/star.png', isImg: true, loop: true, className: 'text-warning' },
            { slug: '1', name: 'Bintang 1', icon: 'https://cdn.masterdiskon.com/masterdiskon/icon/fe/star.png', isImg: true, loop: true, className: 'text-warning' },
        ]
    },
    {
