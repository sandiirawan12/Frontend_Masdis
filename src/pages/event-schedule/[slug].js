...
function Page() {
    ...
    useEffect(() => {
        if (router.isReady) {
            ...
            shopApi.getEventSchedule(access_token, router.query.slug).then(res => {
                if (res.success) {
                ...
                <div className="row">
                    {!isTabletOrMobile &&
                        <div className="col-md-4">
                            ...
                            <NavRoot>
                                {tags?.map(item =>
                                    <li onClick={() => router.push(`/event-schedule/${item.name}`)} className={item?.name === router.query.slug ? 'active' : ''}>{item?.name}</li>
                                )}
                            </NavRoot>
            ...
            <BottomSheet open={openFilter} onClose={() => setOpenFilter(o => !o)}>
                <SheetContent className='pt-3 pl-3 pr-3 pb-5'>
                    ...
                    <ul className="list-group pl-0 bg-danger">
                        {tags?.map(item => (
                            <li onClick={() => router.push(`/event-schedule/${item.name}`)} className={`list-group-item border-0 py-2 pl-0 font-weight-bold my-0 ${item.name === router.query.slug && 'text-primary'}`}>{item.name}</li>
                        ))}
                    </ul>
                </SheetContent>
            </BottomSheet>
