...
function WidgetHotel(props) {
    ...
    const handleChangeKeyword = (q) => {
        if (q.length >= 3) {
            ...
            homeApi.getAutoCompleteElastic({ query: q, latitude: latUser, longitude: lngUser })
            .then(res => {
                if (res.success) {
    ...
    //         // Assuming homeApi.getAutoCompleteElastic returns a Promise
    //         homeApi.getAutoCompleteElastic({ query: keyword, latitude: latUser, longitude: lngUser })
    //             .then(res => {
    //                 if (res.success) {
    //                     setListHotel(res.data);
    //                 }
    ...
    //     // Clear the timeout if the user continues to type within the 2 seconds
    ...
    useEffect(() => {
        const timerId = setTimeout(() => {
            if (options?.from || productId) {
                homeApi.getAutoCompleteElastic({ query: keyword, latitude: latUser, longitude: lngUser }).then(res => {
                    if (res.success) {
                        let selected = [];
                        if (productId) {
