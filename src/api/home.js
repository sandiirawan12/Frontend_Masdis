...
const homeApi = {
    ...
    getHotelTerdekat: (lat, lon) => {
        ...
        return fetch(`${api_masterdiskon_com}apitrav/booking/hotelTerdekat?lat=${lat}&lang=${lon}&limit=50`).then(res => res.json())
    },
    ...
    getMaskapaiFlight: (token) => {
        return fetch(`${api_masterdiskon_com}product/flight/maskapai`, {
            headers: { Authorization: `Bearer ${token}` }
        }).then(res => res.json())
    },
    ...
    getAutoCompleteElastic: (params) => {
        const linkElasticSearch = "https://elasticsearch.masterdiskon.com/"
        return fetch(`${linkElasticSearch}search?q=${params.query}&latitude=${params.latitude}&longitude=${params.longitude}`)
        .then(res => res.json())
        .catch(error => console.error(error));
    },
    getAutoCompleteElasticFlight: (params) => {
        const linkElasticSearch = "https://elasticsearch.masterdiskon.com/"
        return fetch(`${linkElasticSearch}autocompleteFlight?q=${params.query}`)
        .then(res => res.json())
        .catch(error => console.error(error));
    },
