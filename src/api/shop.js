...
const shopApi = {
    ...
    getFlightProductsOther: (token, options, filters) => {
        const req = {
            ...options,
            pax: {
                adult: Number(options['adult']),
    ...
    updateSeat: (token, req) => {
        return fetch(`${staging_masterdiskon_com}KAI/seat/update-seat`, {
            method: 'put',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
            body: JSON.stringify(req)
        }).then(res => res.json())
    ...
    getGeneralProducts: (token, options, filters) => {
        return fetch(`${api_masterdiskon_com}product?page=${filters.page}&limit=${filters.limit}&keyword=`, {
            headers: { Authorization: `Bearer ${token}` }
        }).then(res => res.json())
    },
    ...
    addReview: (data, token) => {
        return fetch(`${api_masterdiskon_com}order/createReview`, {
            headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
            method: 'post',
