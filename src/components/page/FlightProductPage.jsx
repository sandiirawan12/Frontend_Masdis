...
function reducer(state, action) {
    switch (action.type) {
        ...
        case 'FETCH_FLIGHT_OTHER_SUCCESS':
            return { ...state, listFlightOther: action.payload, isLoading: false }
...
function FlightProductPage() {
    ...
    useEffect(() => {
        dispatch({ type: 'FETCH_FLIGHT' })
        shopApi.getFlightProductsOther(access_token, state.options, { ...state.filters, statSort }).then(res => {
            if (res.success) {
                if (flightChoose.flight1) {
                    dispatch({ type: 'FETCH_FLIGHT_OTHER_SUCCESS', payload: res.data.optionsRt })
                } else {
                    dispatch({ type: 'FETCH_FLIGHT_OTHER_SUCCESS', payload: res.data.options })
                }
            }
        })
    }, [state.options, state.filters]);
    ...
    const renderContentOther = () => {
        return state.listFlightOther?.length > 0 ? state.listFlightOther?.map((item, index) =>
            isTabletOrMobile ?
                <CardFlightMobile handleSelected={handleSelected} isLoading={state.isLoading} key={index} index={index} flight={item} />
                :
                <CardFlight handleSelected={handleSelected} type="other" isLoading={state.isLoading} key={index} index={index} flight={item} />)
            :
            <div className="pesawat-opsi-dep">
                <div>
                </div>
            </div>
    }
    ...
    const renderContent = () => {
        return state.listFlight?.length > 0 ? state.listFlight?.map((item, index) =>
            isTabletOrMobile ?
                <CardFlightMobile handleSelected={handleSelected} isLoading={state.isLoading} key={index} index={index} flight={item} />
                :
                <CardFlight handleSelected={handleSelected} isLoading={state.isLoading} key={index} index={index} flight={item} />)
            :
            <div className="pesawat-opsi-dep">
                <div className="text-center mt-5 mb-5">
                    ...
                    <h5>Hasil pencarian lain tidak ditemukan</h5><p>Silahkan perbaharui pencarian Anda.</p>
                </div>
            </div>
    }
                                    ...
                                    <div>
                                        {state.isLoading ?
                                            ...
                                            <p style={{ fontSize: '16px' }} className="pt-3">
                                                Pencarian anda dari
                                                <br />
                                                <span class="text-primary font-weight-bold">{detailSearch.from}</span> ke <span class="text-primary font-weight-bold">{detailSearch.to}</span>
                                            </p>
                                        }
                                    </div>
                                    <div>
