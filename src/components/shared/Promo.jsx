...
function Promo(props) {
    ...
    return <>
        ...
        <Swiper slidesPerView={isTabletOrMobile ? 2 : 3} spaceBetween={10}>
            {data?.map(item => (
                <SwiperSlide key={item.name} style={{ background: 'white', padding: '5px', borderRadius: '10px', boxShadow: '-1px 3px 11px -7px rgba(156,156,156,0.75)'}}>
                    <Link href={`/promo/${item.slug}`}>
                        <CardStyle>
                            <img src={`${item.image}`} alt={item.name} style={{ borderRadius: '10px', width: '100%', minHeight: '80px' }} layout="fill" />
                        </CardStyle>
                    </Link>
                </SwiperSlide>
            ))}
