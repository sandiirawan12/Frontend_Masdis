                ...
                <div className="row">
                    <div className='col-8'>
                        <div className="card" style={{ borderRadius: '20px', boxShadow: '5px 5px 10px -5px rgba(0,0,0,.5)' }}>
                            <div className="card-body">
                                ...
                                <Swiper spaceBetween={5} style={{
                                    ...
                                    {
                                        [...product?.product_img, { img_url: product.img_featured_url }].filter(item => item !== null).map(item => (
                                            <SwiperSlide style={{ width: '100%', height: '350px' }}>
                                                <img style={{ width: '100%', height: '100%', objectFit: 'cover' }} src={item.img_url} />
                                            </SwiperSlide>
                                        ))
                                    }
                                </Swiper>
                                ...
                                <div className='mt-3'>
                                    {product?.tag.map(item => (
                                        <Link href={`/product/category/${product?.product_category?.slug_product_category}?filter_tag=${item?.slug_product_tag}`}>
                                            <a className="btn btn-light border btn-sm mr-2"><span >#{item?.name_product_tag}</span></a>
                                        </Link>
                                    ))}
                                </div>
                        ...
                        <div className="card my-3" style={{ borderRadius: '20px', boxShadow: '5px 5px 10px -5px rgba(0,0,0,.5)' }}>
                            ...
                            <div className="card-body">
                                <div className="media">
                                    <img style={{ height: '80px', width: '80px' }} className="rounded-circle img-fluid" src="https://cdn.masterdiskon.com/masterdiskon/product/property/2021/property-1614152451240.jpg" />
                                    <div className="media-body ml-3">
                                        <b>
                                            <Link href={`/partner/${product?.id_store}`}><a className='text-primary' >{
                                                product?.vendor.display_name}</a></Link></b><br />
                                        <b>{product.city.id_city}</b><br />
                                ...
                                <div className="text-center">
                                    <div>
                                        {product?.product_detail[0]?.discount ?
                                            <>
                                                <small><s>Rp{product?.product_detail[0]?.normal_price?.toLocaleString()}</s></small>
                                                <span className="h4 mx-2"><b className="text-primary">Rp{product?.product_detail[0].price.toLocaleString()}</b></span>

                                            </>
                                            :
                                            <span className="h4"><b className="text-primary">Rp{product?.product_detail[0]?.normal_price?.toLocaleString()}</b></span>
                                        }
                                        {Number(product?.product_detail[0]?.discount ?? 0) > 0 &&
                                            <span className="badge badge-danger">{product?.product_detail[0]?.discount}% OFF</span>
                                        }
                                    </div>
                                    {
                                        Number(product?.product_detail[0]?.discount ?? 0) > 0 &&
                                        <small>Hemat <b className="text-danger">Rp{product?.product_detail[0]?.saving?.toLocaleString()}</b></small>
                                    }
