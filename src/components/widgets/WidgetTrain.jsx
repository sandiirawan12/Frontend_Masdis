...
function WidgetTrain(props) {
  ...
  const handleSearch = () => {
    ...
    const futureDate = moment(today).add(90, 'days').toDate();

    if (stateFrom.selected.length === 0) {
      toast.error(`Stasiun asal belum diisi`)
    } else if (stateTo.selected.length === 0) {
      toast.error(`Stasiun tujuan belum diisi`)
    } else if (data.infant > data.adult) {
      toast.error(`Bayi tidak boleh melebihi jumlah dewasa`)
    } else if (data.adult > 4) {
      toast.error(`Jumlah dewasa maksimal 4`)
    } else if (date.from > futureDate) {
      toast.error(`Jadwal yang tersedia yaitu maksimal 90 hari dari tanggal hari ini`);
    } else if (stateFrom.selected.length > 0 && stateTo.selected.length > 0) {
      const params = {
        ...passanger,
